...
def verify_translation(original, translation):
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user",
             "content": f"Original sentence in hindi: {original},user translation: {translation}. based on the original sentance and the user generated translation guide, tell what is wrong and right. "}
        ]
    )
    print(completion.choices[0].message.content)
    return completion.choices[0].message.content.strip()
...
def app():
    ...
    st.header('Reading Interpretation & Translation')
